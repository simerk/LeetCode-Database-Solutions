...
Group all events by session and calculate:
...
Then filter only the sessions that satisfy all the required conditions.
...
WITH cte AS (
    ...
    SUM(event_type = 'scroll') AS cts_ratio,
...
1. The `cte` groups all events by `session_id` and `user_id`.
2. `TIMESTAMPDIFF()` calculates the session duration using the earliest and latest event timestamps.
3. `SUM(event_type = 'click')` counts the number of click events.
4. `SUM(event_type = 'scroll')` counts the number of scroll events.
5. `cts_ratio` is calculated as:

   ```
   Click Events / Scroll Events
   ```

6. `SUM(event_type = 'purchase')` counts the number of purchase events.
7. Filter only the sessions that:
   - Last more than **30 minutes**.
   - Have at least **5 scroll events**.
   - Have a click-to-scroll ratio less than **0.20**.
   - Have **no purchase events**.
8. Sort the result by `scroll_count` (descending) and `session_id` (ascending).
...
- `n` = number of rows in the `app_events` table.

**Space Complexity:** `O(n)`

- Used to store the aggregated statistics for each session in the CTE.
